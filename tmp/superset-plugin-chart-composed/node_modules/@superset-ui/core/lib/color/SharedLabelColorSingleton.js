...
export class SharedLabelColor {
    ...
    getColorMap(colorNamespace, colorScheme, updateColorScheme) {
        if (colorScheme) {
            ...
            if (sharedLabels.length) {
                ...
                while (analogousColors[analogousColors.length - 1]?.length) {
                    analogousColors.forEach(colors => generatedColors.push(colors.shift()));
                }
                sharedLabelMap = sharedLabels.reduce((res, label, index) => ({
                    ...res,
                    [label.toString()]: generatedColors[index]?.toHexString(),
                }), {});
            }
            const labelMap = Object.keys(this.sliceLabelColorMap).reduce((res, sliceId) => {
                const colorScale = categoricalNamespace.getScale(colorScheme);
                return {
                    ...res,
                    ...Object.keys(this.sliceLabelColorMap[sliceId]).reduce((res, label) => ({
                        ...res,
...
//# sourceMappingURL=SharedLabelColorSingleton.js.map